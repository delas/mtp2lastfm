		...
		: QThread(parent)
{
	...
	m_password = DBConfig::get("SCROBBLER_PASSHASH").value();
	m_username = DBConfig::get("SCROBBLER_USERNAME").value();
	m_uri = DBConfig::get("SCROBBLER_URI").value();
...
void Scrobbler::run()
{
	...
	int out = lastfm_handshake(m_uri);
...
int Scrobbler::lastfm_handshake(QString& uri)
{
	/* get the timestamp */
