...
//								   i, title.toStdString().c_str());
	...
	if (DBConfig::get("DB_VERSION").value() == "1.1")
	{
		// from this to 1.2, new server for scrobbling
		DBConfig::get("DB_VERSION").value("1.2");
		DBConfig::get("SCROBBLER_USERNAME").value(DBConfig::get("LASTFM_USERNAME").value());
		DBConfig::get("SCROBBLER_PASSHASH").value(DBConfig::get("LASTFM_PASSHASH").value());
		DBConfig::get("SCROBBLER_URI").value(LASTFM_HANDSHAKE);
	}
